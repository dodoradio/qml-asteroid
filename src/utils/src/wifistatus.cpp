...
WifiStatus::WifiStatus(QObject *parent) : QObject(parent), mBus(QDBusConnection::systemBus())
{
    mPowered = false;
    mConnected = false;
    ...
    mWatcher = new QDBusServiceWatcher("", QDBusConnection::systemBus());
    ...
    QDBusInterface remoteOm("", "/", "org.freedesktop.DBus.ObjectManager", mBus);
    if(remoteOm.isValid())
        serviceRegistered("");
    else
...
{
    mBus.connect("", "/", "org.freedesktop.DBus.ObjectManager", "InterfacesAdded", this, SLOT(InterfacesAdded(QDBusObjectPath, InterfaceList)));
    mBus.connect("", "/", "org.freedesktop.DBus.ObjectManager", "InterfacesRemoved", this, SLOT(InterfacesRemoved(QDBusObjectPath, QStringList)));
